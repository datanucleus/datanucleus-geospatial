 ...
 */
public class SpatialIsEmptyMethod2 extends AbstractSQLMethod
{
    /* (non-Javadoc)
    ...
    public SQLExpression getExpression(SQLExpression expr, List args)
    {
        ...
        {
            throw new NucleusUserException("Cannot invoke Spatial.isEmpty without 1 argument");
        }
        else if (expr != null && args != null && args.size() != 0)
        {
            throw new NucleusUserException("Cannot invoke geom.isEmpty() with an argument");
        }
        ...
        GeometryExpression geomExpr = new GeometryExpression(stmt, 
            SpatialMethodHelper.getGeometryMapping(clr, argExpr), "geometry.from_sdo_geom", geomFuncArgs, null);

        ArrayList funcArgs = new ArrayList();
        funcArgs.add(geomExpr);
        return SpatialMethodHelper.getBooleanExpression(stmt, "isEmpty", funcArgs, exprFactory);
    }
}